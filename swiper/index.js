...
Component({
	...
	methods: {
			...
			if (startTime == 0) {
				...
				timer = setTimeout(function () {
					...
					if (that.data.clickNum == 1) {
						...
						that.trigger(e,"doubleTap");
					} else {
						// console.log("单击执行事件");
						
						that.trigger(e,"singleTap");
						if (isPlay) {
							if (player.src) {
								ctx.pause();
							}
		...
		trigger(e, type) {
			...
			this.triggerEvent(type, Object.assign(Object.assign({}, detail), {
				video: curVideo,
				ctx:this._videoContexts[e.currentTarget.dataset.playerIdx]
			}, ext));
		},
		playCurrent(cur) {
			const {
